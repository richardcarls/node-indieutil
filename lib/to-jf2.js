...
module.exports = function toJf2(mf2, options) {
  ...
  options = options || {};
  if (options.preferredContentType !== false) {
    if (!_.isString(options.preferredContentType)) {
      options.preferredContentType = 'text/html';
    }
  }
  if (options.implicitContentType !== false) {
    if (!_.isString(options.implicitContentType)) {
      options.implicitContentType = 'text/plain';
    }
  }
  if (!_.isPlainObject(options.compact)) {
  ...
  function compact(item) {
    ...
    if (!options.compact) {
      excluded = compactExclude[item.type];
    }
    
    if (_.isPlainObject(options.compact)) {
      excluded = options.compact[item.type];
    }
